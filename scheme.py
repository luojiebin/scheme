...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(scheme_eval(first, env),
                            rest,
                            env)
        # END PROBLEM 5
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        # BEGIN PROBLEM 3
        if isinstance(symbol, (int, float)) or symbol is nil:
            return symbol
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        while args is not nil:
            python_args.append(scheme_eval(args.first, env))
            args = args.second
        # BEGIN PROBLEM 4
        if self.use_env:
    ...
    def make_call_frame(self, args, env):
        ...
        while args is not nil:
            if isinstance(args.first, Pair):
                ...
                val = env.lookup(args.first)
            vals.append(val)
            args = args.second

        val_pair = nil
        for v in reversed(vals):
            val_pair = Pair(v, val_pair)
        return self.env.make_child_frame(self.formals, val_pair)
        # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not scheme_symbolp(name):
            raise SchemeError('the first element of let item must be a symbol')
        formals = Pair(name, formals)
        val = bindings.first.second.first
        values = Pair(scheme_eval(val, env), values)
...
class MuProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        for v in reversed(vals):
            val_pair = Pair(v, val_pair)
        return env.make_child_frame(self.formals, val_pair)
    # END PROBLEM 16
